...
[source,bash]
----
javac src/module-info.java src/com/printer/impl/Printer.java -d dist/printer-impl <1>

javac src/com/printer/client/PrinterClient.java -d dist/printer-client <2>
----
...
In the client application, we have two simple clasess GetClient and PostClient. They simple requests to the HTTP/2 server we run in previous step and makes HTTP Get or Post requests.
...
Let's run the GetClient with required trustStore arguments.
...
After run, verify that version information, http status code, and response body correctly printed.
